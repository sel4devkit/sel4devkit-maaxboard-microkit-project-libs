                     ...
                     const uint8_t hwdst_addr[ETH_HWADDR_LEN], const uint32_t ipdst_addr)
{
    if (net_queue_empty_free(&tx_queue)) {
        printf("ARP|LOG: Transmit free queue empty or transmit active queue full. Dropping reply\n");
        return -1;
    }
...
seL4_MessageInfo_t protected(microkit_channel ch, microkit_msginfo msginfo)
{
    ...
    case REG_IP:
        printf("ARP|NOTICE: client%d registering ip address: %s with MAC: %02lx:%02lx:%02lx:%02lx:%02lx:%02lx\n",
        ...
        break;
    default:
        printf("ARP|LOG: PPC from client%d with unknown message label %lu\n", client,
